...
class TestEc < Minitest::Test


    def create_self_signed_ca_certificate(random_key, expires)
        key = OpenSSL::PKey::EC.generate random_key
    ...
    def setup
        @cert, @key = create_self_signed_ca_certificate('prime256v1', 3600)
        @other_cert, @other_key = create_self_signed_ca_certificate('prime256v1', 3600)
    end
